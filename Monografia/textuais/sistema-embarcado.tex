...
A implementação do buzzer é simples e orientada a eventos. Para cada estado o buzzer dispara uma sequencia de bipes no momento da transição, sendo sendo 4 bipes longos indicativos de erro na inicialização, 3 bipes curtos indicativo de entrada no estado de calibração, 1 bipe longo indicativo de estado de \textit{idle}, e 2 bipes curtos indicativo de inicialização da telemetria. A task do buzzer passará 99\% do seu tempo de vida no estado \textit{Blocked} aguardando um comando na fila. Quando a instrução chegar, a task acorda, executa o som e volta a dormir.

\subsection{Leitura dos sensores}
Para os sensores, as implementações passam a ser bem mais complexas com baixo nível de abstração. 
Camada de adapter e ports para os sensores

\subsection{Tratamento de dados na ponta}
Filtros, edge computing, quantização para redução de carga na rede
...
\section{Protótipo}
Processo de fabricação, metodologias de teste e estimativa de consumo elétrico
