...
Generated from: ('crow_telemetry.xml,minimal.xml',)
...
# CROW_COMMAND_TYPE
enums["CROW_COMMAND_TYPE"] = Enum()
enums["CROW_COMMAND_TYPE"].bitmask = False
CROW_CMD_IDLE = 0
enums["CROW_COMMAND_TYPE"][0] = EnumEntry("CROW_CMD_IDLE", """""")
CROW_CMD_START_RECORDING = 1
enums["CROW_COMMAND_TYPE"][1] = EnumEntry("CROW_CMD_START_RECORDING", """""")
CROW_CMD_STOP_RECORDING = 2
enums["CROW_COMMAND_TYPE"][2] = EnumEntry("CROW_CMD_STOP_RECORDING", """""")
CROW_COMMAND_TYPE_ENUM_END = 3
enums["CROW_COMMAND_TYPE"][3] = EnumEntry("CROW_COMMAND_TYPE_ENUM_END", """""")
...
MAVLINK_MSG_ID_CROW_IMU = 201
MAVLINK_MSG_ID_CROW_BARO = 202
MAVLINK_MSG_ID_CROW_GPS = 203
MAVLINK_MSG_ID_CROW_COMMAND = 204
MAVLINK_MSG_ID_HEARTBEAT = 0


class MAVLink_crow_imu_message(MAVLink_message):
    """
    Dados quantizados da IMU e Magnetômetro
    """

    id = MAVLINK_MSG_ID_CROW_IMU
    msgname = "CROW_IMU"
    fieldnames = ["time_boot_ms", "accel_x", "accel_y", "accel_z", "gyro_x", "gyro_y", "gyro_z", "mag_x", "mag_y", "mag_z"]
    ordered_fieldnames = ["time_boot_ms", "accel_x", "accel_y", "accel_z", "gyro_x", "gyro_y", "gyro_z", "mag_x", "mag_y", "mag_z"]
    ...
    def __init__(self, time_boot_ms: int, accel_x: int, accel_y: int, accel_z: int, gyro_x: int, gyro_y: int, gyro_z: int, mag_x: int, mag_y: int, mag_z: int):
        MAVLink_message.__init__(self, MAVLink_crow_imu_message.id, MAVLink_crow_imu_message.msgname)
        self._fieldnames = MAVLink_crow_imu_message.fieldnames
        self._instance_field = MAVLink_crow_imu_message.instance_field
        self._instance_offset = MAVLink_crow_imu_message.instance_offset
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.time_boot_ms, self.accel_x, self.accel_y, self.accel_z, self.gyro_x, self.gyro_y, self.gyro_z, self.mag_x, self.mag_y, self.mag_z), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_crow_imu_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_crow_baro_message(MAVLink_message):
    """
    Dados quantizados do Barômetro
    """

    id = MAVLINK_MSG_ID_CROW_BARO
    msgname = "CROW_BARO"
    fieldnames = ["time_boot_ms", "pressure_delta", "temperature"]
    ordered_fieldnames = ["time_boot_ms", "pressure_delta", "temperature"]
    fieldtypes = ["uint32_t", "int16_t", "int16_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {}
    native_format = bytearray(b"<Ihh")
    orders = [0, 1, 2]
    lengths = [1, 1, 1]
    array_lengths = [0, 0, 0]
    crc_extra = 89
    ...
    def __init__(self, time_boot_ms: int, pressure_delta: int, temperature: int):
        MAVLink_message.__init__(self, MAVLink_crow_baro_message.id, MAVLink_crow_baro_message.msgname)
        self._fieldnames = MAVLink_crow_baro_message.fieldnames
        self._instance_field = MAVLink_crow_baro_message.instance_field
        self._instance_offset = MAVLink_crow_baro_message.instance_offset
        self.time_boot_ms = time_boot_ms
        self.pressure_delta = pressure_delta
        self.temperature = temperature

    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.time_boot_ms, self.pressure_delta, self.temperature), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_crow_baro_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_crow_gps_message(MAVLink_message):
    """
    Dados de GPS preservando alta acurácia (Ponto Fixo)
    """

    id = MAVLINK_MSG_ID_CROW_GPS
    msgname = "CROW_GPS"
    fieldnames = ["time_boot_ms", "latitude", "longitude", "altitude_msl", "ground_speed", "satellites"]
    ordered_fieldnames = ["time_boot_ms", "latitude", "longitude", "altitude_msl", "ground_speed", "satellites"]
    fieldtypes = ["uint32_t", "int32_t", "int32_t", "uint16_t", "uint16_t", "uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {}
    ...
    def __init__(self, time_boot_ms: int, latitude: int, longitude: int, altitude_msl: int, ground_speed: int, satellites: int):
        MAVLink_message.__init__(self, MAVLink_crow_gps_message.id, MAVLink_crow_gps_message.msgname)
        self._fieldnames = MAVLink_crow_gps_message.fieldnames
        self._instance_field = MAVLink_crow_gps_message.instance_field
        self._instance_offset = MAVLink_crow_gps_message.instance_offset
        self.time_boot_ms = time_boot_ms
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.time_boot_ms, self.latitude, self.longitude, self.altitude_msl, self.ground_speed, self.satellites), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_crow_gps_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_crow_command_message(MAVLink_message):
    """
    Comando de alteração de estado do sistema
    """

    id = MAVLINK_MSG_ID_CROW_COMMAND
    msgname = "CROW_COMMAND"
    fieldnames = ["time_boot_ms", "command"]
    ordered_fieldnames = ["time_boot_ms", "command"]
    fieldtypes = ["uint32_t", "uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {"command": "CROW_COMMAND_TYPE"}
    fieldunits_by_name: Dict[str, str] = {}
    ...
    def __init__(self, time_boot_ms: int, command: int):
        MAVLink_message.__init__(self, MAVLink_crow_command_message.id, MAVLink_crow_command_message.msgname)
        self._fieldnames = MAVLink_crow_command_message.fieldnames
        self._instance_field = MAVLink_crow_command_message.instance_field
        self._instance_offset = MAVLink_crow_command_message.instance_offset
        self.time_boot_ms = time_boot_ms
        self.command = command

    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.time_boot_ms, self.command), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_crow_command_message, "name", mavlink_msg_deprecated_name_property())
...
mavlink_map: Dict[int, Type[MAVLink_message]] = {
    MAVLINK_MSG_ID_CROW_IMU: MAVLink_crow_imu_message,
    MAVLINK_MSG_ID_CROW_BARO: MAVLink_crow_baro_message,
    MAVLINK_MSG_ID_CROW_GPS: MAVLink_crow_gps_message,
    MAVLINK_MSG_ID_CROW_COMMAND: MAVLink_crow_command_message,
...
class MAVLink(object):
    ...
    def crow_imu_encode(self, time_boot_ms: int, accel_x: int, accel_y: int, accel_z: int, gyro_x: int, gyro_y: int, gyro_z: int, mag_x: int, mag_y: int, mag_z: int) -> MAVLink_crow_imu_message:
        """
        Dados quantizados da IMU e Magnetômetro

        time_boot_ms              : Timestamp (ms) (type:uint32_t)
        accel_x                   : X accel (m/s^2 * 100) (type:int16_t)
        accel_y                   : Y accel (m/s^2 * 100) (type:int16_t)
        accel_z                   : Z accel (m/s^2 * 100) (type:int16_t)
        gyro_x                    : X gyro (deg/s * 10) (type:int16_t)
        gyro_y                    : Y gyro (deg/s * 10) (type:int16_t)
        gyro_z                    : Z gyro (deg/s * 10) (type:int16_t)
        mag_x                     : X mag (raw LSB) (type:int16_t)
        mag_y                     : Y mag (raw LSB) (type:int16_t)
        mag_z                     : Z mag (raw LSB) (type:int16_t)

        """
        return MAVLink_crow_imu_message(time_boot_ms, accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z, mag_x, mag_y, mag_z)

    def crow_imu_send(self, time_boot_ms: int, accel_x: int, accel_y: int, accel_z: int, gyro_x: int, gyro_y: int, gyro_z: int, mag_x: int, mag_y: int, mag_z: int, force_mavlink1: bool = False) -> None:
        """
        Dados quantizados da IMU e Magnetômetro
        ...
        """
        self.send(self.crow_imu_encode(time_boot_ms, accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z, mag_x, mag_y, mag_z), force_mavlink1=force_mavlink1)

    def crow_baro_encode(self, time_boot_ms: int, pressure_delta: int, temperature: int) -> MAVLink_crow_baro_message:
        """
        Dados quantizados do Barômetro

        time_boot_ms              : Timestamp (ms) (type:uint32_t)
        pressure_delta            : Pressure offset (Pa) (type:int16_t)
        temperature               : Temperature (C * 100) (type:int16_t)

        """
        return MAVLink_crow_baro_message(time_boot_ms, pressure_delta, temperature)

    def crow_baro_send(self, time_boot_ms: int, pressure_delta: int, temperature: int, force_mavlink1: bool = False) -> None:
        """
        Dados quantizados do Barômetro
        ...
        """
        self.send(self.crow_baro_encode(time_boot_ms, pressure_delta, temperature), force_mavlink1=force_mavlink1)

    def crow_gps_encode(self, time_boot_ms: int, latitude: int, longitude: int, altitude_msl: int, ground_speed: int, satellites: int) -> MAVLink_crow_gps_message:
        """
        Dados de GPS preservando alta acurácia (Ponto Fixo)

        time_boot_ms              : Timestamp (ms) (type:uint32_t)
        latitude                  : Latitude (deg * 1E7) (type:int32_t)
        longitude                 : Longitude (deg * 1E7) (type:int32_t)
        altitude_msl              : Altitude MSL (m * 10) (type:uint16_t)
        ground_speed              : Ground speed (m/s * 10) (type:uint16_t)
        satellites                : Number of satellites visible (type:uint8_t)

        """
        return MAVLink_crow_gps_message(time_boot_ms, latitude, longitude, altitude_msl, ground_speed, satellites)

    def crow_gps_send(self, time_boot_ms: int, latitude: int, longitude: int, altitude_msl: int, ground_speed: int, satellites: int, force_mavlink1: bool = False) -> None:
        """
        Dados de GPS preservando alta acurácia (Ponto Fixo)
        ...
        """
        self.send(self.crow_gps_encode(time_boot_ms, latitude, longitude, altitude_msl, ground_speed, satellites), force_mavlink1=force_mavlink1)

    def crow_command_encode(self, time_boot_ms: int, command: int) -> MAVLink_crow_command_message:
        """
        Comando de alteração de estado do sistema

        time_boot_ms              : Timestamp (ms) (type:uint32_t)
        command                   : Estado comandado (type:uint8_t, values:CROW_COMMAND_TYPE)

        """
        return MAVLink_crow_command_message(time_boot_ms, command)

    def crow_command_send(self, time_boot_ms: int, command: int, force_mavlink1: bool = False) -> None:
        """
        Comando de alteração de estado do sistema
        ...
        """
        self.send(self.crow_command_encode(time_boot_ms, command), force_mavlink1=force_mavlink1)
